...
test("POST /movies/:id/actors should set the movies actors", async () => {
    const actor = await Actor.create({
        firstName: "José Mov-Act",
        lastName: "Ruiz Mov-Act",
        nationality: "Venezuelan Mov-Act",
        image: "",
        birthday: "1980-12-14"
    })
    const res = await request(app)
...
test("POST /movies/:id/directors should set movies directors", async() => {
    const director = await Director.create({
        firstName: "José Mov-Dir",
        lastName: "Ruiz Mov-Dir",
        nationality: "Venezuelan Mov-Dir",
        image: "",
        birthday: "1980-12-14"
    })
    const res = await request(app)
